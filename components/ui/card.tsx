...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card flex flex-col gap-4 overflow-hidden rounded-lg bg-card py-6 text-sm text-card-foreground shadow-xl ring-1 ring-foreground/10 squircle has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl',
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex items-center rounded-b-xl border-t bg-muted/50 px-6 py-4 group-data-[size=sm]/card:p-3',
        className
      )}
      {...props}
    />
